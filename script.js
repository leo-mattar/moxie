...
// MOMENT.JS - NEW YORK CURRENT TIME
function updateDisplay() {
  const newYorkTime = moment().tz("America/New_York");

  $("[ny-time]").text(newYorkTime.format("hh:mm A"));
}
...
function clientsMarquee() {
  ...
  let marquerDuration = $(".c-marquee-item").length * 5;

  tl.fromTo(
    ".c-marquee-wrap",
    { xPercent: 0 },
    { xPercent: 100, duration: marquerDuration, ease: "none" }
  );
}
